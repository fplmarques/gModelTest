...
#
#  Usage:
#       $ gmodel_test.pl [options] aligned_sequences.fas
# 
system clear;
use Getopt::Long;
use POSIX qw(ceil);
...
if ($fixed_ML_GTR_topology =~ /1/){
    ...
    \n";
    close (OUT);
    # end of writing garli configuration file
    ...
    $tree = 1;
    } # this is the conditional operator for fixed topologies, also controlls values of K
#
print "     ######################################################################\n";
...
for $ratematrix ("\(a a a a a a\)","\(a b a a b a\)","\(a b a a c a\)","\(a b c c b a\)","\(a b a c b c\)","\(a b c a b c\)","\(a b c c d a\)","\(a b a c d c\)","\(a b c a d c\)","\(a b c d b e\)","\(a b c d e f\)"){
    if ($run_fork =~ /1/){my $pid = $pm->start and next;}
    for $statefrequencies ("equal","estimate"){
        for $invariantsites ("none","estimate"){
            for $ratehetmodel ("none","gamma"){
                ...
                #############################  printing progress report for serial execution only
                if ($run_fork =~ /0/){
            ...
            } # for loop for $ratehetmodel

        } # for loop for $invariantsites
    } # for loop for $statefrequencies
    if ($run_fork =~ /1/){$pm->finish;}
} # for loop for $ratematrix
if ($run_fork =~ /1/){$pm->wait_all_children;}
#
##
#
print "     ######################################################################\n";
